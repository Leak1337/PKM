...
namespace PokemonGo.RocketAPI.Console
{
    public partial class Pokemons : Form
    {
        ...
        {
            EnabledButton(false, "Lade Pokemon Liste neu.");
            ...
            try
            {
                ...
                Text = "Pokemon Liste | Benutzer: " + profile.Profile.Username + " | Pokemon: " + pokemons.Count() + "/" + profile.Profile.PokeStorage;
                EnabledButton(true);
            ...
            {
                Logger.ColoredConsoleWrite(ConsoleColor.Red, "Fehler lade die Liste neu: " + e.Message);
                Execute();
            }
        }
        ...
        private async void toolStripMenuItem1_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(this, pokemon.PokemonId + " mit " + pokemon.Cp + " CP " + Math.Round(pokemon.CalculateIV()) + "% perfekt", "Bist du sicher das du es verschicken willst?", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
            ...
            if (resp.Status)
            {
                ...
                Text = "Pokemon List | Benutzer: " + profile.Profile.Username + " | Pokemon: " + PokemonListView.Items.Count + "/" + profile.Profile.PokeStorage;
            }
            else
                MessageBox.Show(resp.Message + " Verschicken gescheitert!", "Transfer Status", MessageBoxButtons.OK);
        }
        ...
        private async void btnEvolve_Click(object sender, EventArgs e)
        {
            ...
            foreach (ListViewItem selectedItem in selectedItems)
            {
                ...
                {
                    evolved++;
                    statusTexbox.Text = "Entwickle..." + evolved;
                }
                else
                    failed += resp.Message + " ";
            }
            ...
            if (failed != string.Empty)
                MessageBox.Show("Erfolgreich entwickelt " + evolved + "/" + total + " Pokemon. Failed: " + failed, "Evolve status", MessageBoxButtons.OK, MessageBoxIcon.Information);
            else
                MessageBox.Show("Erfolgreich entwickelt " + evolved + "/" + total + " Pokemon.", "Evolve status", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (evolved > 0)
            {
                PokemonListView.Clear();
                Execute();
        ...
        {
            EnabledButton(false, "Verschicke...");
            var selectedItems = PokemonListView.SelectedItems;
            int transfered = 0;
            int total = selectedItems.Count;
            string failed = string.Empty;
            ...
            foreach (ListViewItem selectedItem in selectedItems)
            {
                ...
                if (resp.Status)
                {
                    ...
                    transfered++;
                    statusTexbox.Text = "Verschicke..." + transfered;
                }
                else
                    failed += resp.Message + " ";
        ...
        private async void evolveToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(this, pokemon.PokemonId + " mit " + pokemon.Cp + " CP " + Math.Round(pokemon.CalculateIV()) + "% perfekt", "Bist du sicher das du es entwickeln willst?", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
        ...
        private async void powerUpToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(this, pokemon.PokemonId + " mit " + pokemon.Cp + " CP " + Math.Round(pokemon.CalculateIV()) + "% perfekt", "Bist du sicher das du es Leveln willst?", MessageBoxButtons.OKCancel) == DialogResult.OK)
            {
        ...
        {
            EnabledButton(false, "Level up...");
            DialogResult result = MessageBox.Show("Dieser process dauert ein wenig.", "FullPowerUp status", MessageBoxButtons.OKCancel, MessageBoxIcon.Information);
            if (result == DialogResult.OK)
            {
                ...
                while (i == 0)
                {
                    ...
                    if (failed != string.Empty)
                    {
                        if (powerUps > 0)
                        {
                            MessageBox.Show("Pokemon erfolgreich gelevelt " + powerUps + " times up.", "FullPowerUp status", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        else
                        {
                            MessageBox.Show("Pokemon nicht enwickelt. Nicht genügen Sternenstaub oder Candys.", "FullPowerUp status", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                        i = 1;
                        EnabledButton(true);
                    }
                    ...
                    {
                        powerUps++;
                        statusTexbox.Text = "Level up..." + powerUps;
                    }
                }
                if (powerdup > 0 && i == 1)
                {
